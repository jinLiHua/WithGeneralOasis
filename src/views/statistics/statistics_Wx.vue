          ...
          <el-tabs v-model="activeName" @tab-click="handleClick" type="card">
            ...
            <el-tab-pane label="表格数据" name="first">
              <el-row style="padding: 10px;background-color: #fff">
                <el-table :data="list"
                  ...
                  <el-table-column prop="repairNum" label="维修点数">
                  </el-table-column>
                  <el-table-column prop="repairInFive" label="5日内维修点数">
                  </el-table-column>
                  <el-table-column prop="repairInFifteen" label="15日内维修点数">
                  </el-table-column>
                  <el-table-column prop="repairToNow" label="至今修复点数">
                  </el-table-column>
                </el-table>
              </el-row>
            ...
            <el-tab-pane label="图表数据" name="second">
              ...
              <el-row style="padding: 0">
                  <div id="main1" class="chart_main" style="height: 400px;background-color: #fff;border-bottom: 10px solid #f5fcf5"></div>
                  <div id="main2" class="chart_main" style="height: 400px;background-color: #fff; padding-top: 10px"></div>
              </el-row>
            </el-tab-pane>
    ...
    methods: {
      ...
      drawingChart() {
        this.$nextTick(function() {
          this.drawPieChart('main1', '维修点数', this.chartOptions, this.repairChartData);
          this.drawBarChart('main2', this.chartOptions,  this.barChartData);
        })
      },
      drawBarChart(id, options, chartData) {
        this.charts = echart.init(document.getElementById(id));
        this.charts.setOption({
          color: ['#003366', '#006699', '#ED5A24', '#88D93E',],
          tooltip: {
            trigger: 'axis',
            axisPointer: {
              type: 'shadow'
      ...
      drawPieChart(id, titile, options, optionsData){
        this.charts = echart.init(document.getElementById(id));
        this.charts.setOption({
          color:this.colors,
          tooltip: {
      ...
      getList(){
        ...
        query.statisticsRepair(this.listQuery).then(res => {
          ...
          data.forEach(function (e) {
            options.push(e.type);
            repairData.push({value:e.repairNum, name:e.type},);
            repairBarData[0].push(e.repairNum);
            repairBarData[1].push(e.repairInFive);
            repairBarData[2].push(e.repairInFifteen);
            repairBarData[3].push(e.repairToNow);
          })
          that.barChartData.titles.forEach(function (e, index) {
            barChartData.push({name:e, type: 'bar', label: that.labelOption, data:repairBarData[index]})
          })
          that.barChartData.series = barChartData;
//          饼图数据
          that.repairChartData = repairData;
          that.chartOptions = options;
...
//            sums[index] += ' ';
    ...
    data() {
      return {
        ...
        repairChartData: [],
        barChartData: {
          titles:['维修点数','5日内维修点数','15日内维修点数','至今修复点数',],
          series:[],
        },
