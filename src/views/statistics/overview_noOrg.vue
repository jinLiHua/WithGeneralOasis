<template>
  <div class="app-container org_container">
    <el-container>
      <el-main>
  ...
  export default {
    methods: {
      ...
      getList(){
        ...
        query.emissionOverview(this.listQuery).then(res => {
          let data = res.data;
          let tableFormat = [];
          let currentList = [];
          for (let i = 0; i < data.length; i++) {
            ...
            tableFormat.push(currentData.length);
          }
      ...
      getSummaries(param) {
        ...
        columns.forEach((column, index) => {
          ...
          if (!values.every(value => isNaN(value))) {
           ...
           if (index === 2) {
             this.allArrivePoint = sums[index];
           } else if (index === 4) {
             this.allLeakPoint = sums[index];
           } else if (index === 6) {
             this.maintenancePoints = sums[index];
           }
            if (index === 5) {
              sums[index] = this.allLeakPoint/this.allArrivePoint * 100;
              sums[index] = sums[index].toFixed(2)
            }
            if (index === 7) {
              sums[index] = this.maintenancePoints/this.allLeakPoint * 100;
              sums[index] = sums[index].toFixed(2)
            }
            if(index == 8 || index == 9){
            ...
            if(index == 11){
                 sums[index]=  (sums[10]/ sums[8]*100).toFixed(4);
            }
            sums[index] += '';
          } else {
            sums[index] = 'N/A';
